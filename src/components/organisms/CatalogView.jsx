...
export default function CatalogView() {
    ...
    const filteredCatalog = useMemo(() => {
        ...
        return items.filter(item => {
            ...
            // Handle compound categories like "History/Politics"
            const itemCategories = item.category ? item.category.split('/').map(s => s.trim()) : [];
            const matchesCategory = selectedCategory === "All" || itemCategories.includes(selectedCategory);

            return matchesSearch && matchesCategory;
        });
    }, [data.catalog, search, selectedCategory]);
